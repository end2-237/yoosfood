      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          {current.items.map((item) => (
            ...
            >
              <Link href={`/produit/${item.slug}`} className="flex min-w-0 flex-1 items-center gap-4">
                <Dish
                  src={item.img}
                  alt={item.name}
                  Icon={item.Icon}
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="truncate text-base font-black text-gray-900 transition group-hover:text-red-600">{item.name}</h3>
